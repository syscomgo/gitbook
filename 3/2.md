...
本文件以Ubuntu 16.04 , 20.04 和 CentOS 8(若無特別標示Ubuntu版本，則為一樣的步驟)，
...
Ubuntu 16.04 Python 及必要套件安裝範例

( 請注意是否有使用到原先的 Python，<mark style="color:red;">**建立虛擬環境時請使用正確的 Python**</mark> )

```
add-apt-repository ppa:jonathonf/python-3.8
add-apt-repository ppa:deadsnakes/ppa
apt-get update -y
apt-get install -y python3.8python3.8-dev apache2-devpython3-venv python3.8-venv
```

{% hint style="danger" %}
<mark style="color:red;">**Ubuntu 16.04若有使用到環境中原有 Python，請斟酌考量以下操作。**</mark>

刪除原先的 Python(請視環境中Python的版本做刪除)

apt remove -y python3.5-minimal python3.5 python2.7&#x20;

將 Python 3.8 設定為預設Python

update-alternatives --install /usr/bin/python3 pythons/usr/bin/python3.8 1
{% endhint %}

Ubuntu 20.04 必要套件安裝範例

```
apt-get update -y
apt-get install -y python3.8-dev apache2-devpython3-venv python3.8-venv
```

CentOS Python 及必要套件安裝範例&#x20;

```
dnf -y install python38gcc httpd-devel python38-devel redhat-rpm-config
```
...
#### 啟用 django.conf 設定檔 <a href="#qi-yong-djangoconf-she-ding-dang" id="qi-yong-djangoconf-she-ding-dang"></a>

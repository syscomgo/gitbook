...
7. フォーム番号：data\_noとも言え、1つのプロセスに1つのフォーム番号しかありません。
8. データ番号：id / data\_idとも言え、1つのプロセスに複数のidがあり、推進ごとに新しいidが生成する。
...
1. **URL**：対象のOMFLOW ServerのURL、例：https://omflow.com.tw
2. **api\_path**：プロセスのAPI pathを指定する
...
1. **URL**：対象のOMFLOW ServerのURL、例：https://omflow.com.tw
2. **api\_path**：プロセスのAPI pathを指定する 。
3. **data\_id**：データ番号について、OMFLOWのプロセスは「並行」機能があったため、「フォーム番号」だけで正しく推進できない可能性があります。返事するリストデータの中、keyはidの值であり、data\_idとなります。
...
1. **URL**：対象のOMFLOW ServerのURL、例：https://omflow.com.tw
2. **token**：doLoginで取得したToken。
3. **api\_path**：プロセスのAPI pathを指定する。
4. **data\_no**：検索するフォーム番号。
...
2. **api\_path**：プロセスのAPI pathを指定する。
3. **token**：doLoginで取得したToken。
4. **data\_no**：入力するデータタイプはArrayListで、リストに削除するフォーム番号番号を入れる。
...
**searchkeyとsearch\_conditionsはいずれかが有効になる (search\_conditionsの方が優先)**

| properties         | method         | input type                             | note                        |
| ------------------ | -------------- | -------------------------------------- | --------------------------- |
| api\_path          | setAPI         | String                                 | 　　　                         |
| searchkey          | setSearchkey   | String                                 | search\_columnsに対してあいまい検索する |
...
| ------------- | ---------------------- | ---------------------------------- | ------------------------------------------------ |
| name          | setColumnName          | String                             | 　　　　　　　                                          |
| is\_query     | getIsQuery             | boolean                            | setStringValueにFalseを返事する，setQueryValueにTrue返事する |

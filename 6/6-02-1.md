...
例えば、test\_flow.omfは繁体字中国語で設計されたアプリケーションであり、既にOMFLOWの内蔵機能によって各言語の翻訳がアップロードされています。英語を話す顧客がこのアプリケーションを利用する必要がある場合、プログラムで「デザイン」の言語を繁体字中国語から英語に変更することができます。
...
PDF ソフトウェア： 公式サイトから（ https://wkhtmltopdf.org ）現在の環境に適用できるインストールファイルをダウンロードし、インストールしてください。
...
2. 'formitm\_1' フィールドイコール'SYSCOM' の記録だけを検索したい場合は、以下の内容を追加します。

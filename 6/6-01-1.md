...
```
参照箇所：［メインメニュー］＞[アプリケーション管理]＞[アプリケーション設計]＞［任意のアプリ］＞［任意のフロー］＞[フロー設計]タブ＞[コード]コンポーネント
```
ここではPythonコードを提供し、ユーザーは[コード]コンポーネントで直接OMFLOWの機能を操作できます。説明は下記のとおりです。
...
#レスポンスの取得
#レスポンスの詳細は REST API紹介>フォームのクエリを参照してください
```
...
シナリオ1、フォーム番号イコール5のフローをクエリして処理する
listの書き方：
condition = []
...
シナリオ3、フォーム番号が5より大きいフローをクエリし、かつ、ある入力フィールドの値イコールtestのフローを処理する
listの書き方：
condition = []
...
user_id = '1'                  #ユーザ番号
files = None                   #ファイル
update_duplicate = False       #条件付きクエリによって返されたデータが複数となる場合、すべて処理するか。もしそうでなければ、すべてを処理せず、失敗のメッセージを送り返す。
update_duplicate_interval = 0  #処理するデータが複数となる場合、処理ごとの間隔(秒単位)
...
#レスポンスの取得
status = result['status']      #状態True/False
message = result['message']    #メッセージを返す。処理成功data_idまたは失敗のメッセージを送り返す。
```
...
#修正するユーザーを選択する(必要、user_idまたはusernameのいずれを入力)
user_id = None
username = None
...
#レスポンスの取得(クエリフィールドに従ってデータを取得してください)
#resultデータの構成は以下のとおりです
{
    ...
    'roles':[],                 #groupsはユーザー所属のID配列をレスポンスし、rolesはユーザーの役割のID配列をレスポンスします
}
```
...
#resultが空文字列の場合、削除が成功したことを意味し、テキストメッセージがレスポンスされる場合はエラーメッセージの意味です。
```
...
#クエリ対象の職務名、職務コードや権限名を入力(必須、3つのいずれを入力)
position_name = None
position_no = None
...
#入力情報(オプション)
parent_group_id = None         #親部門id、役割は入力不可
description = ''               #說明フィールド
group_no = ''                  #部門コード
order = 0                      #部門別順番、数字が小さいものを先にして、整数を入力してください（マイナスも可）
...
#１つの部門(役割)のオブジェクトを宣言する
group_id = ''
group_no = ''
group_name = ''
#値は部門id/部門コード/部門名の3つのいずれを入力
...
permissions = result.get('permissions','')           #該当の役割の権限、部門クエリする際のレスポンスは空文字列
...
job_role_list = [
  {
    'job_role_id': <データ番号>,   #古いデータを更新する際に入力
    'position_no': <職務コード>,
    'user_id': <ユーザー番号>,
    'responsibility': <権限>, 
    'data_index': <ソート>
...
org_chart_dict = {
    'org_name': <組織図名>,
    'value': [{
            'org_id': <組織ンポーネント番号>, #古いデータを更新する際に入力
            'group_no': <組織コンポーネントコード>,
            'group_name': <組織コンポーネント名>,
            'parent_group_no': <親組織コンポーネントコード>,
            'order': <組織コンポーネントソート>,
    ...
    'is_sub': <委託管理される組織図かどうか>, #True/False
    'uid': <uid>,
    'org_config_id': <組織図番号> #古いデータを更新する際に入力
}
